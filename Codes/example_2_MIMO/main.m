...
lambda_1 = 2*diag([1,1]);
lambda_2 = 0.01*diag([1,1]);
lambda_3 = 0.02*diag([1,1]);
...
x_11 = zeros(1,nSteps+1);
x_11(1) = 0.5;
x_11(2) = 0.5;

x_12 = zeros(1,nSteps+1);
x_12(1) = 0;
x_12(2) = 0;

x_21 = zeros(1,nSteps+1);
x_21(1) = 0.5;
x_21(2) = 0.5;

x_22 = zeros(1,nSteps+1);
x_22(1) = 0;
x_22(2) = 0;
...
y_d(:,1) = [ 0.6 + 0.2*(sin(2*pi*(1)*Ts/50)+sin(2*pi*(1)*Ts/100)+sin(2*pi*(1)*Ts/150))
             0.6 + 0.1*(cos(2*pi*(1)*Ts/50)+sin(2*pi*(1)*Ts/120)+cos(2*pi*(1)*Ts/150))];
y_d(:,2) = [ 0.6 + 0.2*(sin(2*pi*(2)*Ts/50)+sin(2*pi*(2)*Ts/100)+sin(2*pi*(2)*Ts/150))
             0.6 + 0.1*(cos(2*pi*(2)*Ts/50)+sin(2*pi*(2)*Ts/120)+cos(2*pi*(2)*Ts/150))];
...
PHI_hat{1} = [repmat(0.5*diag([1,1]),1,Ly) repmat(0.5*diag([1,1]),1,Lu)];
PHI_hat_y = cell(1,nSteps);
PHI_hat_y{1} = repmat(0.5*diag([1,1]),1,Ly);
PHI_hat_y_1 = cell(1,nSteps);
PHI_hat_y_1{1} = 0.5*diag([1,1]);
PHI_hat_u = cell(1,nSteps);
PHI_hat_u{1} = repmat(0.5*diag([1,1]),1,Lu);
PHI_hat_u_1 = cell(1,nSteps);
PHI_hat_u_1{1} = 0.5*diag([1,1]);
PHI_hat_u_others = cell(1,nSteps);
PHI_hat_u_others{1} = repmat(0.5*diag([1,1]),1,Lu-1);
...
for k = 2:nSteps
    ...
    %% Reference signals
%     y_d(:,k+1) = [ 0.4*(k<=500) + 0.7*(k>500 & k<=1000) + 0.5*(k>1000 & k<=1500);
%                    0.6*(k<=300) + 0.8*(k>300 & k<=700) + 0.7*(k>700 & k<=1200) + 0.5*(k>1200 & k<=1500) ];

%     y_d(:,k+1) = [ 0.4
%                    0.6 ];
               
%     y_d(:,k+1) = [ 0.6 + 0.2*(sin(200*pi*(k+1)*Ts) + 0*0.25*sin(100*pi*(k+1)*Ts) + 0.02*sin(66.7*pi*(k+1)*Ts))
%                    0.6 + 0.2*(sin(133.2*pi*(k+1)*Ts) + 0.15*sin(80*pi*(k+1)*Ts) + 0.25*sin(250*pi*(k+1)*Ts)) ];
               
    y_d(:,k+1) = [ 0.6 + 0.2*(sin(2*pi*(k+1)*Ts/50)+sin(2*pi*(k+1)*Ts/100)+sin(2*pi*(k+1)*Ts/150))
                   0.6 + 0.1*(cos(2*pi*(k+1)*Ts/50)+sin(2*pi*(k+1)*Ts/120)+cos(2*pi*(k+1)*Ts/150))];
               
%     y_d(:,k+1) = [ 0.4*(k<200) + (0.3*sin(k*pi/100))*(k>=200)
%                    0.2*(sin((2*pi/50)*(k+1))+sin((2*pi/100)*(k+1))+sin((2*pi/150)*(k+1))); ];
...
%     delta_u_sw(:,k) = ((PHI_hat_u_1{k}+m_u_1)^-1)*(lambda_sw*sat_func(s(:,k),10));
    ...
    %% Plant simulation
    dist(:,k) = [ 0.05*sin(pi*k/200)
                  0.08*sin(2*pi*k/200) ];
              
    a = 1 + 0.1*sin(2*pi*k/200);
    b = 1 + 0.1*cos(2*pi*k/200);
    
    x_11(k+1) = ((x_11(k)^2)/(1+x_11(k)^2)) + 0.3*x_12(k);
    x_12(k+1) = ((x_11(k)^2)/(1+x_12(k)^2+x_21(k)^2+x_22(k)^2)) + a*u(1,k);
    x_21(k+1) = ((x_21(k)^2)/(1+x_21(k)^2)) + 0.2*x_22(k);
    x_22(k+1) = ((x_21(k)^2)/(1+x_11(k)^2+x_12(k)^2+x_22(k)^2)) + b*u(2,k);
        
    y_1 = x_11(k+1) + dist(1,k);
    y_2 = x_21(k+1) + dist(2,k);
...
PHI_hat_y_1_mat_22 = PHI_hat_y_1_mat(2,2:nInputs:end);
...
grid on;
xlabel('Time (s)','FontName','Times','FontSize',14);
ylabel('$\Delta{s}_2$ (k)','Interpreter','LATEX','FontName','Times','FontSize',14);
...
grid on;
xlabel('Time (s)','FontName','Times','FontSize',14);
ylabel('$e_2$ (k)','Interpreter','LATEX','FontName','Times','FontSize',14);
% legend('Upper prescribed boundary','Tracking error','Lower prescribed boundary','FontName','Times','FontSize',14);
...
axes('Position',[0.27086383601757 0.130544487799413 0.163250366032211 0.085500592787076]);
interval_2 = 440:640;
...
grid on;
xlabel('Time (s)','FontName','Times','FontSize',14);
ylabel('$u_2$ (k)','Interpreter','LATEX','FontName','Times','FontSize',14);
...
grid on;
xlabel('Time (s)','FontName','Times','FontSize',14);
ylabel('$y_2$ (k)','Interpreter','LATEX','FontName','Times','FontSize',14);
% legend('Desired','Actual','Location','SouthEast','FontName','Times','FontSize',14);
lg_handle_3_2 = legend('Desired','Actual');
lg_handle_3_2.Location = 'SouthEast';
